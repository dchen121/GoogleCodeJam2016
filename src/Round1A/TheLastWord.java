...
/**
 * Created by Daniel on 2016-04-15.
 */
public class TheLastWord {
     ...
     * the letters of S, one by one, in the order in which they appear in S. When the host presents the first letter,
     * the contestant writes it on the whiteboard; this counts as the first word in the game (even though it is only one
     ...
     * y is the winning last word, as described in the statement.
     *
     * Limits
     * 1 ≤ T ≤ 100.
     *
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= t; ++i) {
            ...
            String answer = generateLastWord(s);

            System.out.println("Case #" + i + ": " + answer);
        }
    }

    private static String generateLastWord(String s) {
        if (s.length() == 0 || s.length() == 1) {
            return s;
        }

        String lastWord = s.substring(0, 1);
